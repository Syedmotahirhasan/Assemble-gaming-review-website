...
if (isset($_GET['delete_comment'])) {
    ...
    if ($comment) {
        ...
        if ($can_delete) {
            ...
            header('Location: '.$_SERVER['PHP_SELF'].'?id='.$review_id.'#comments');
            exit;
        }
    }
}
...
if (isset($_POST['edit_comment_id']) && isset($_POST['edit_comment_text'])) {
    ...
    if ($comment) {
        ...
        if (!empty($_SESSION['user_name']) && $_SESSION['user_name'] === $comment['user_name']) {
            $can_edit = true; // User or admin can edit own
        }
        if ($can_edit) {
...
<script>
// Restore scroll position IMMEDIATELY after DOM loads, before any anchor scroll
window.addEventListener('DOMContentLoaded', function() {
    ...
    if (scrollY !== null) {
        ...
        }, 10); // Give DOM a moment to render
    }
});
</script>
<script>
